...
llm = ChatOllama(model="llama3.2", base_url=OLLAMA_URL, num_predict=1024,)

PROMPT_TEMPLATE = """
You are a helpful assistant. Answer the question using ONLY the context below.
Always respond in the same language as the question.
If the answer is not in the context, say "Nie wiem / I don't know".
...
def query_rag(question: str) -> Generator:
    ...
    sources = "\n".join([
        ...
        for doc in results
        if not (os.path.basename(doc.metadata['source']) in seen
                or seen.add(os.path.basename(doc.metadata['source'])))
    ])
